...
export function SelectedPhotos({ photos, coverId, disabled, onPick, onRemove, onCover }: {
  ...
  return <View style={themed_styles.section}>
    <Text style={themed_styles.title}>Add Photos</Text>
    <Text style={themed_styles.hint}>Optional · Choose a cover for your garage. Photos are saved when you save the vehicle.</Text>
    ...
    <View style={themed_styles.grid}>
      {photos.map((photo, index) => <View key={photo.id} style={themed_styles.tile}>
...
function Preview({ uri }: { uri: string }) {
  ...
  return <View style={themed_styles.preview}>{failed
    ? <Text style={themed_styles.hint}>Preview unavailable. Remove and select again.</Text>
...
function Action({ label, accessibilityLabel, disabled, selected, onPress }: {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={accessibilityLabel ?? label}
    ...
    style={[themed_styles.button, selected && themed_styles.selected, disabled && { opacity: 0.5 }]}>
    <Text style={themed_styles.buttonText}>{label}</Text>
  </Pressable>;
}
